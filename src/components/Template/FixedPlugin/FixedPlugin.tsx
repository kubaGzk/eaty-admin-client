...
export default function FixedPlugin({ ...props }: any) {
  const [classes, setClasses] = React.useState<any>('dropdown show');
  const [bg_checked, setBg_checked] = React.useState<any>(true);
  const [bgImage, setBgImage] = React.useState<any>(props.bgImage);
  const handleClick = () => {
